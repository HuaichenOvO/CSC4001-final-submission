    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not (User.objects.filter(username=uname).exists()):
                ...
                link = f'https://have-you-eaten.herokuapp.com/verify/{token}'
                ...
                try:
                    ...
                    context = {"title":"Invalid Registration"}
...
def tasks(request):
    ...
    if request.method == "POST":
        ...
        if cmd_name == "Delete":
            ...
            new_order.save()
            task.delete()
            messages.success(request, 'Claimed')
...
def delivery(request):
    ...
    deliveries1 = Order.objects.filter(
        (Q(sender=request.user.id) & Q(status="Pending"))|
        (Q(sender=request.user.id) & Q(status="Sending"))|
        (Q(sender=request.user.id) & Q(status="Claiming")) )
    deliveries2 = Order.objects.filter((Q(sender=request.user.id) & Q(status="Canceled")) | (Q(sender=request.user.id) & Q(status="Done")))
...
def order(request):
    ...
    orders2 = Order.objects.filter((Q(buyer=request.user.id) & Q(status="Canceled")) | (Q(buyer=request.user.id) & Q(status="Done")))
